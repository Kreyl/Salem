...
uint32_t ICalcDelayN(uint32_t n, uint32_t ASmoothValue) {
    uint32_t DelayR = (LedWs.ICurrentClr[n].R == DesiredClr[n].R)? 0 : CalcDelay(LedWs.ICurrentClr[n].R, ASmoothValue);
    uint32_t DelayG = (LedWs.ICurrentClr[n].G == DesiredClr[n].G)? 0 : CalcDelay(LedWs.ICurrentClr[n].G, ASmoothValue);
    uint32_t DelayB = (LedWs.ICurrentClr[n].B == DesiredClr[n].B)? 0 : CalcDelay(LedWs.ICurrentClr[n].B, ASmoothValue);
...
#else
#define FLASH_COLOR             ((Color_t){255,255,255})
#endif
